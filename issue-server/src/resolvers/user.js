  ...
  Query: {
    ...
    allUnassignedUsers: async (_, args, { models }) => {
      const assigned = await models.Project.findAll({
        where: { isActive: true },
        include: [{ model: models.User, as: 'member' }],
      });
      const users = new Set(
        assigned
          .map((project) => project.member.map((member) => member.id))
          .flat()
      );
  ...
  Mutation: {
    createUser: async (_, args, { models, cloudinary }) => {
      ...
      try {
        ...
        const userExist = await models.User.findOne({where:{username:args.username}});
    ...
    loginUser: async (_, args, { models }) => {
      try {
        const user = await models.User.findOne({
          where: { username: args.username, isVerified: true },
        });
    ...
    sendForgotPasswordEmail: async (_, args, { models }) => {
      ...
      const token = jwt.sign(
        { exp: Date.now() + 3600000, data: userForToken },
        process.env.SECRET
      );
      try {
