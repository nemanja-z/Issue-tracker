  ...
  Mutation: {
    createProject: async (_, args, { models, user }) => {
      ...
      const projectLead = await models.User.findOne({
        where: { username: args.projectLead },
      });
    ...
    addMember: async (_, { username, project }, { models, user }) => {
      ...
      if (!targetProject) {
        throw new Error(
          'You cannot add a member because project doesn\'t exist or you are not a manager!'
        );
      }
      if (!addUserRole) {
    ...
    changeStatus: async (_, args, { models, user }) => {
      ...
      if (!project) {
        throw new Error(
          'The project manager can only change the project status!'
        );
      }
      try {
